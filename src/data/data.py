...
def readCSVCanData(path: str, nrows: int):
    ...
    if dirname == "CSU":
        ...
        df = readCSV(path, ";", nrows, 4, 2, loxamDateParser, r"\b[0-9A-Fa-f]+\b", extraTimepos=1)
...
def readCSV(path: str, sep: str, nrows: int, idpos: int, timepos: int, dateParser, idMatch: str, extraTimepos: int=-1):
